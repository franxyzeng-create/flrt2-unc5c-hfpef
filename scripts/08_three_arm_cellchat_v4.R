###############################################################################
# Step 2b-2(v4):三臂 CellChat 环境RNA稳健性检验 —— 出版级
#
# v4 相比 v3 的修改(按专家第三轮审查):
#   ⑨ 方向计算 tapply 的 NA→0:某通路在某组无检出,tapply 返回 NA(非0),
#      会让"HFpEF富集、control无检出"的通路 direction=NA→误判不稳健。
#      这可能正中 axon-guidance 头条(增强=control可能测不出)。补0修正。
#   ⑩ 加 HFpEF_specific flag:control无检出的通路其 rankNet p 是构造性p=0
#      (单组退化非真检验),与"两组都有、真Wilcoxon显著"的稳健通路分开标注。
#      报告时这类描述成"仅HFpEF检出"而非"Wilcoxon显著增强"。
#   ⑪ decont 行列【按名对齐】(不假定顺序)+ 保序断言,防静默贴错标签。
#
# (v3 已落实:方向判定、raw臂本脚本重建、cell set一致、idents levels一致、
#  每通路一个p、HEADLINE名核对、DecontX小数不取整、三臂raw.use=TRUE、cb臂原样进@data)
#
# 三臂(只换输入矩阵,参数全锁死):
#   raw       : raw_count_cellranger → normalizeData(log1p CP10K@1e4)
#   cellbender: raw_count_cellbender(已 log1p CP10K@1e4)→ 原样进@data,跳过normalizeData
...
# ⑪ 行列【按名对齐】而非假定顺序;先断言长度一致
stopifnot(ncol(dx$counts_decont_raw) == length(dx$cellnames),
          nrow(dx$counts_decont_raw) == nrow(sce),
          length(dx$group) == length(dx$cellnames),
          length(dx$celltype) == length(dx$cellnames))
decont_counts <- as(dx$counts_decont_raw, "CsparseMatrix")  # 小数,不取整
# 若 07b 给矩阵设了 dimnames,验证保序;否则赋名(DecontX 在 cellranger 矩阵上跑,保序)
if (!is.null(colnames(decont_counts))) {
  stopifnot(identical(colnames(decont_counts), dx$cellnames))
} else {
  colnames(decont_counts) <- dx$cellnames
}
if (is.null(rownames(decont_counts))) rownames(decont_counts) <- rownames(sce)
stopifnot(identical(rownames(decont_counts), rownames(sce)))
...
get_rank_dir_bh <- function(merged_obj) {
  ...
  flow[is.na(flow)] <- 0     # ⑨ 某组无检出=信息流0(非NA),否则方向被误判NA→误伤HFpEF富集通路
  ...
  # ⑩ control 无检出 = 仅HFpEF检出:其 rankNet p 是构造性p=0(单组退化非真检验)
  dir_df$HFpEF_specific <- dir_df$flow_control == 0 & dir_df$flow_HFpEF > 0
...
look_spec<- function(bh) bh$HFpEF_specific[match(HEADLINE, bh$name)]
...
# 仅HFpEF检出标记(三臂任一为TRUE即提示该通路在该臂是"仅HFpEF",p为构造性)
summ$HFpEF_specific_any <- look_spec(bh_raw) | look_spec(bh_cb) | look_spec(bh_dx)
...
cat("HFpEF_specific_any=TRUE: 该通路在某臂仅 HFpEF 检出(control=0),其 rankNet p 是\n")
cat("  构造性 p=0(单组退化、非真分布检验)。报告时描述为'仅在 HFpEF 检出',\n")
cat("  而非'经 Wilcoxon 显著增强'——与'两组都有、真 Wilcoxon 显著'的稳健通路分开主张。\n")
